...
- **Governing ADRs:** ADR-001, ADR-008, ADR-009
...
Changes to these paths may contradict ratified architecture. The control plane
watches them; it does not block them.

```cdad-drift-signals
paths: infra/**, deploy/**, docker-compose*.yml, **/migrations/**, **/Dockerfile
manifests: package.json, requirements.txt, pyproject.toml, pom.xml, go.mod, Cargo.toml
config: .env.example, **/application*.yml, **/appsettings*.json
```

The keys are free-form labels and appear in the warning message. The values are
comma-separated globs. Adjust them to the project's real stack — in this repo,
which has no application yet, they stay as an example.
...
|---|---|---|
| 2026-09-08 | ADR-008 | None — governance-model change, no stack row applies |
| 2026-09-08 | ADR-009 | Added the Drift signals view: declares the paths outside the governed tree that carry architectural weight |
