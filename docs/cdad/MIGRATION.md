...
| L0 file list repeated 6 times across 4 files | Stated once, in `CLAUDE.md` |
| `Proposed Architecture Change` template duplicated in 3 files | One copy, in the `cdad-propose-change` skill |
| L0–L3 table in both `governance.md` and `project-context.md` | One copy, in `docs/cdad/METHODOLOGY.md` |
...
| v1 | v2 |
|---|---|
| `cdad/AGENTS.md` | `CLAUDE.md` (rules) + `docs/cdad/METHODOLOGY.md` (rationale) |
| `cdad/ai-rules.md` | `CLAUDE.md` + `.claude/skills/cdad-propose-change/` |
| `cdad/governance.md` | `docs/cdad/METHODOLOGY.md` |
| `cdad/guardrails.md` | `.claude/settings.json` + `.claude/rules/` |
| `cdad/project-context.md` | `docs/cdad/METHODOLOGY.md` |
...
1. Copy `CLAUDE.md`, `.claude/`, `cdad/`, and `docs/` into your project root.
...
6. Start a session and run `/context`. Confirm `CLAUDE.md` appears under memory
   files and that nothing from `docs/` or `.claude/skills/` is loaded.
7. Wire `scripts/cdad-check-stack.sh` into CI against your default branch.
...
v2 ships this way already: `AGENTS.md` holds the portable core and `CLAUDE.md`
imports it. Kiro and Codex read `AGENTS.md` natively, so they pick up the core
with no adapter. See `docs/cdad/PORTABILITY.md` for what each tool does and does
not enforce.

If you only ever use Claude Code, you can inline `AGENTS.md` into `CLAUDE.md`
and delete it — but the indirection costs nothing and keeps the door open.
